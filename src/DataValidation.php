 ...
 * @see https://github.com/Kholenkov/php-data-validation/blob/master/src/DataValidation.php
 */
class DataValidation
{
     ...
     * @return boolean
     */
    public static function validateOgrn (string $ogrn, &$error_message = null, &$error_code = null)
    {
        $result = false;
        $ogrn = (string) $ogrn;
        if (!$ogrn) {
     ...
     * @return boolean
     */
    public static function validateSnils (string $snils, &$error_message = null, &$error_code = null) : bool
    {
        $result = false;
        $snils = (string) $snils;
        if (!$snils) {
            ...
            $error_code = 3;
            $error_message = 'СНИЛС может состоять только из 11 цифр! Передан' . Data::getTypeRu($snils);
        } else {
            $sum = 0;
            for ($i = 0; $i < 9; $i++) {
                $sum += (int) $snils[$i] * (9 - $i);
